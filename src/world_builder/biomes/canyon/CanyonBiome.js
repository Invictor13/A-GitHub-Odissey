...
export class CanyonBiome extends BiomeBase {
    ...
    setupMaterials() {
        ...
        // Canyon specific geometries
        this.tallRockGeo = new THREE.CylinderGeometry(0.5, 0.8, 4.0, 6);
        this.tallRockGeo.translate(0, 2.0, 0);
        this.rockGeo = new THREE.IcosahedronGeometry(0.8, 0);
    }
    ...
    build3DGeometry(terrainGroup, chunksList) {
        ...
        const colorBaseFloor = new THREE.Color(0xcd853f); // Peru
        const colorSpot = new THREE.Color(0xd2691e); // Chocolate
        const colorTrail = new THREE.Color(0x8b4513);
        ...
        for (let cx = 0; cx < numChunksX; cx++) {
            for (let cz = 0; cz < numChunksZ; cz++) {
                ...
                const rockMatrices = [], tallRockMatrices = [];
                ...
                for (let x = startX; x < endX; x++) {
                    for (let z = startZ; z < endZ; z++) {
                        ...
                        if (cell.type === this.TILE_SOLID) {
                            if (cell.distToPath <= 4 && Math.random() < 0.2) { // Tall rocks
                                ...
                                dummy.scale.set(1.0 + Math.random(), 1.0 + Math.random()*2.0, 1.0 + Math.random());
                                dummy.updateMatrix(); tallRockMatrices.push(dummy.matrix.clone());
                            }
                            if (cell.distToPath <= 3 && Math.random() < 0.3) { // Normal rocks
                ...
                if (tallRockMatrices.length > 0) {
                    const iTallRock = new THREE.InstancedMesh(this.tallRockGeo, this.matRock, tallRockMatrices.length);
                    tallRockMatrices.forEach((m, i) => iTallRock.setMatrixAt(i, m)); iTallRock.castShadow = true; iTallRock.receiveShadow = true; chunkGroup.add(iTallRock);
                }
        ...
        if (this.map.rooms && this.map.rooms.length > 1) {
            for (let rIdx = 1; rIdx < this.map.rooms.length; rIdx++) {
                ...
                if (this.grid[spawnX] && this.grid[spawnX][spawnZ] && this.grid[spawnX][spawnZ].type === this.TILE_FLOOR) {
                    ...
                    const enemy = Math.random() < 0.6 ? new Lizardman(this.scene, new THREE.Vector3(px, py, pz)) : new Kobold(this.scene, new THREE.Vector3(px, py, pz));

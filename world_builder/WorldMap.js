...
export class WorldMap {
    constructor(scene) {
        ...
        this.bridgeMat = new THREE.MeshStandardMaterial({ color: palettes.bridge, roughness: 1.0 });
        ...
        // Efeito Mágico
        this.magicRingGeo = new THREE.TorusGeometry(1, 0.1, 8, 32);
        this.magicRingMat = new THREE.MeshBasicMaterial({ color: 0x60a5fa, transparent: true, opacity: 0, side: THREE.DoubleSide });
        this.magicRing = new THREE.Mesh(this.magicRingGeo, this.magicRingMat);
        ...
        this.cloudGeo = new THREE.PlaneGeometry(1200, 1200, 40, 40);
        this.cloudGeo.rotateX(-Math.PI / 2);
        this.seaOfClouds = new THREE.Mesh(this.cloudGeo, this.cloudSeaMat);
        this.seaOfClouds.position.y = -35;
        this.worldGroup.add(this.seaOfClouds);
        ...
        this.worldGroup.add(this.topCloudIM);
        this.worldGroup.add(this.bottomCloudIM);
    ...
    initWorld() {
        ...
        setTimeout(() => this.spawnIsland(1, 0, 0, 0), 1000);
        setTimeout(() => this.spawnIsland(0, 1, 0, 0), 1800);
        setTimeout(() => this.spawnIsland(-1, 0, 0, 0), 2600);
        setTimeout(() => this.spawnIsland(0, -1, 0, 0), 3400);
    }
    ...
    openModal(data) {
        ...
        document.getElementById('m-title').innerText = this.getRandomObj(b.names);
        document.getElementById('m-diff').innerText = this.getRandomObj(b.diff);
        document.getElementById('m-enemies').innerText = this.getRandomObj(b.enemies);
        document.getElementById('m-animals').innerText = this.getRandomObj(b.animals);
        document.getElementById('m-fishes').innerText = this.getRandomObj(b.fishes);
        document.getElementById('m-minerals').innerText = this.getRandomObj(b.minerals);
    ...
    exploreIsland() {
        const data = this.activeIslandData;
        if (data) {
            this.playCreationEffect(data.center);

            const clickedMesh = this.interactableObjects.find(obj => obj.userData.gridX === data.gridX && obj.userData.gridZ === data.gridZ);
            if(clickedMesh) clickedMesh.userData.isClickable = false;

            setTimeout(() => {
                const dirs = [ {dx: 1, dz: 0}, {dx: -1, dz: 0}, {dx: 0, dz: 1}, {dx: 0, dz: -1} ];
                dirs.forEach((dir, index) => {
                    setTimeout(() => this.spawnIsland(data.gridX + dir.dx, data.gridZ + dir.dz, data.gridX, data.gridZ), index * 300);
                });
            }, 400);

            // Move into the island
            const biome = data.biomeId;
            if (window.changeGameState) {
                window.changeGameState('ROGUELIKE', { biome });
            }
        }
    }
    ...
    update(delta, time, camera) {
        ...
        // Vento
        this.windObjects.forEach(w => { w.obj.rotation.z = Math.sin(time * w.speed + w.phase) * 0.06; });

        // Nuvens
        const updateClouds = (dataArr, instMesh) => {
            for(let i=0; i<this.cloudCount; i++) {
                dataArr[i].pos.x += dataArr[i].speed;
                if(dataArr[i].pos.x > 400) dataArr[i].pos.x = -400;

                this.dummy.position.copy(dataArr[i].pos);
                this.dummy.scale.setScalar(dataArr[i].scale);
                this.dummy.updateMatrix();
                instMesh.setMatrixAt(i, this.dummy.matrix);
            }
        ...
        // Deformação do mar de nuvens
        const pos = this.seaOfClouds.geometry.attributes.position.array;
        for(let i=0; i<pos.length; i+=3) {
            pos[i+1] = Math.sin(pos[i] * 0.02 + time * 0.5) * 4 + Math.cos(pos[i+2] * 0.02 + time * 0.4) * 4;
        }
